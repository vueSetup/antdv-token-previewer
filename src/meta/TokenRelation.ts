import type { AliasToken, SeedToken, MapToken } from 'ant-design-vue/es/theme/interface'
...
const getMapRelatedAlias = () => {
  ...
  Object.keys(mapToken).forEach((mapKey) => {
    delete aliasToken[mapKey as keyof AliasToken]
  })
  ...
  Object.keys(mapToken).forEach((mapKey) => {
    ...
    Object.keys(aliasToken).forEach((aliasKey) => {
      if (aliasToken[aliasKey as keyof AliasToken] !== newAlias[aliasKey as keyof AliasToken]) {
...
const getSeedRelatedAlias = (): SeedRelatedAlias => {
  ...
  Object.keys(seedToken).forEach((key) => {
    ...
    if (arr.length) {
      result[key as keyof SeedRelatedAlias] = sortToken(Array.from(new Set(arr)))
    }
  })
  return result
}
